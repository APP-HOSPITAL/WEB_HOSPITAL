...
function EmployeeSelector({ people ,id}) {
  ...
  const AddTask=()=>{
    ...
    try{
      selectedPeople.forEach((item)=>{
        axios.post("https://hospital.somee.com/api/addemployeehopital",{
          "idHospital":id,
    ...
    <div className="p-4 bg-gray-200 rounded-lg">
      ...
      <ul className="space-y-2">
        {people
          ...
          .map((person) => (    
            <li key={person.idEmployee} className="text-blue-600">
              {person.name}{' '}
      ...
      <ul className="space-y-2">
        {selectedPeople.map((person) => (
          <li key={person.idEmployee} className="block text-sm font-medium text-black">
            {person.name}
          </li>
        ))}
      </ul>
